...
function displayNumbersDivisible(divisor) {
    ...
    for (let i = 0; i <= number; i++) {
        if (i % divisor === 0) { // Check if 'i' is divisible by the given divisor
            arrayOfNum.push(i); // Add the number to the array
            sum += i; // Add the number to the sum
        }
    }

    console.log(`The list of numbers under 500 divisible by ${divisor}:\n\n${arrayOfNum.join(", ")}\n\n`);
    console.log(`The sum of all numbers under 500 that are divisible by ${divisor} is:\n\n${sum}`);
}
...
function totalVacationCost() {
    ...
    let destination = prompt("Where do you wish to fly? Please enter your destination:");
    let rentDur = Number(prompt("How many days would you like to rent the car for?"));
...
console.log('Exercise 5 and 6 are completed in separate files as it was a requirement asked on the platform.')
console.log(horizontalLine)
// Exercise 7 : My Book List
console.log('Exercise 7 : My Book List it is done in Elements not in Console so switch to Elements')
// Instructions
...
allBooks.push({
    ...
    author: "J.K. Rowling",
    image: "http://t3.gstatic.com/images?q=tbn:ANd9GcQttbZNVGzKnwLBGXrKBkt0VY2J2DubjnH_lSQrJCzdSf5fe4OK8yUM27EYhteFc68Ii7Xr9g",
    alreadyRead: true
});
...
allBooks.push({
    ...
    author: "J.K. Rowling",
    image: "http://t2.gstatic.com/images?q=tbn:ANd9GcTd-fG9xkzpbR9ug7pe1WHsYud0BN9JgRc3MVWeUfc-pxIJUQN4_TiYrvqUgYwMtTQUfeQz0A",
    alreadyRead: false
});
...
for (let book of allBooks) {
    ...
    div.style = "border:1px solid black; border-radius:5px; margin:10px; height:300px; display:flex; flex-direction:column; align-items:center; justify-content:center; width:240px";
    if (book.alreadyRead) {
        div.style.color = 'red';
    }
